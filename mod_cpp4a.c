...
int readConf(char *cmdName, char *cmdValue, char *cmdLine, int *cmdLineLen,
		int *isFoundCmd, request_rec *r);
typedef struct {
...
static const command_rec cpp4a_directives[] = { AP_INIT_TAKE1("cpp4aEnabled",
		cpp4a_set_enabled, NULL, RSRC_CONF, "Enable or disable mod_example"),
		AP_INIT_TAKE1("cpp4aConf", cpp4a_set_conf, NULL, RSRC_CONF,
				"The configure file"), { NULL } };
...
static int cpp4a_handler(request_rec *r) {
	...
	nameNoSufLen = strlen(r->filename)
			- ((int) (rindex(r->filename, '/') - r->filename) + 1)
			- suffixChLen;
...
#ifdef CPP4A_DEGUG
			ap_rprintf(r,
					"Operation not permitted , %s cannot be create:error code 10000002<br>",
					binBasePath);
			return OK;
#else	
			return HTTP_INTERNAL_SERVER_ERROR;
	...
	if (access(cmdPath, R_OK) != 0) {
		int rst = readConf(cmdName, cmdValue, cmdLine, &cmdLineLen, &isFoundCmd,
				r);
	} else {
...
int loadSharedOb(char *binFullPath, request_rec *r) {
	...
	char* chRst = NULL;
	int (*run)(request_rec, char**);
	int (*close)();
	handle = dlopen(binFullPath, RTLD_LAZY);
	if (handle == NULL) {
#ifdef CPP4A_CC
		ap_rprintf(r, "load shared library %s failure: error 10000006",
				binFullPath);
		return OK;
#else
		return HTTP_INTERNAL_SERVER_ERROR;
	...
	*(void **) (&run) = dlsym(handle, "CPP4A_RUN");
	if (run == NULL) {
#ifdef CPP4A_CC
		ap_rprintf(r, "No CPP4A_RUN function : error code 10000007<br>");
		return OK;
#else
		return HTTP_INTERNAL_SERVER_ERROR;
#endif
	...
	//char *rst=(*run)(r);
	int rst = (*run)(*r, &chRst);
	ap_rprintf(r, "%s", chRst);
	*(void **) (&close) = dlsym(handle, "CPP4A_CLOSE");
	if (close == NULL) {
#ifdef CPP4A_CC
		ap_rprintf(r, "No CPP4A_CLOSE function : error code 10000007<br>");
		return OK;
#else
		return HTTP_INTERNAL_SERVER_ERROR;
#endif
...
module AP_MODULE_DECLARE_DATA cpp4a_module = { STANDARD20_MODULE_STUFF, NULL, /* create per-dir    config structures */
